...
uint32_t
WifiTxParameters::GetCurrentMpduNumber(Mac48Address receiver, uint8_t tid) const
{
    NS_LOG_FUNCTION(this << receiver << +tid);

    const auto infoIt = m_info.find(receiver);
    if (infoIt == m_info.cend())
    {
        return 0;
    }

    const auto seqIt = infoIt->second.seqNumbers.find(tid);
    if (seqIt == infoIt->second.seqNumbers.cend())
    {
        return 0;
    }

    return static_cast<uint32_t>(seqIt->second.size());
}
void
