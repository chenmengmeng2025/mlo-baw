...
// 扫描配置结构体
struct ScanConfig {
    vector<int> N1_values = {6};
    vector<int> N2_values = {4};
    vector<double> nmpdu_sld0_values = {1};
    vector<double> nmpdu_sld1_values = {1};
    vector<int> BAW_values = {256};
    vector<double> R1_values = {344.11765, 412.94118, 458.823532, 516.176472,573.529412,619.411768,688.235296,825.88236,917.647064,1032.352944,1147.058824,1238.823536,1376.470592,};
    vector<double> R2_values = {2161.764708};
    // vector<double> R1_values = {};
    // vector<double> R2_values = {2000};
};
...
double compute_fraction(double p, double W, int K) {
    const double eps = 1e-9;
    double denom = 2.0 * p - 1.0;

    if (abs(denom) < eps) {
        return 1.0 + W * (1.0 + static_cast<double>(K));
    }

    double term = pow(1.0 - p, K + 1) * pow(2.0, K);
    double fraction = W * (p - term) / denom;

    if (!isfinite(fraction)) {
        return W * (1.0 + static_cast<double>(K));
    }

    return fraction;
}

double compute_pM_simplified(double pS, int N, double W_S, int K_S) {
    double fraction_S = compute_fraction(pS, W_S, K_S);
    double base = 1.0 - 2.0 / fraction_S;
    return pow(base, N);
}

double compute_pS_simplified(double pM, double pS, int N, 
                            double W_M, int K_M, double W_S, int K_S) {
    double fraction_M = compute_fraction(pM, W_M, K_M);
    double term1 = 1.0 - 2.0 / fraction_M;
    
    double fraction_S = compute_fraction(pS, W_S, K_S);
    double term2 = 1.0 - 2.0 / fraction_S;    
    return term1 * pow(term2, max(0, N - 1));
}

pair<double, double> solve_p_simplified(int N, int K_M, int K_S, 
                                       double W_M, double W_S,
                                       int max_iter = 2000, 
                                       double tol = 1e-12, 
                                       double damping = 0.4) {
    if (N <= 0) return {0.0, 0.0};
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        double pM_new = compute_pM_simplified(pS, N, W_S, K_S);
        pM_new = clamp(pM_new, eps, 1.0 - eps);

        double pS_candidate = compute_pS_simplified(pM_new, pS, N, W_M, K_M, W_S, K_S);
        pS_candidate = clamp(pS_candidate, eps, 1.0 - eps);
        double pS_new = (1.0 - damping) * pS + damping * pS_candidate;
...
double calc_payload_duration(double nmpdu, double L_subf, double rate) {
    if (nmpdu < 1e-9) return 0.0;
    return (16 + nmpdu * L_subf + 6) / (rate * 13.6) * 13.6;
    // return floor(ceil((16 + nmpdu * L_subf + 6) / (rate * 13.6)) * 13.6);
}

// 计算传输时间 (tau_T)
double calc_tau_T(double nmpdu, double L_subf, double rate, double sigma, double T_OH, double T_PH) {
    double payloadDuration = calc_payload_duration(nmpdu, L_subf, rate);
    double ppduDuration = payloadDuration + T_PH;
    return ppduDuration / sigma + T_OH;
}

// 计算BA时间
double calc_T_BA(double nmpdu, int BAW) {
    if (nmpdu < 1.5) {  // 近似为1
        return 28.0;
    }
...
tuple<double, double, double, double, double, double> evaluate_continuous(
    ...
    for (int i = 0; i < 2; ++i) 
        T_DIFS[i] = params.cfg->T_SIFS + params.cfg->aifsn * params.cfg->sigma;
    
    vector<double> T_BA = {calc_T_BA(n0, params.BAW), calc_T_BA(n1, params.BAW)};
    vector<double> T_BA_sld = {calc_T_BA(params.nmpdu_sld[0], params.BAW), 
                               calc_T_BA(params.nmpdu_sld[1], params.BAW)};
    
    vector<double> T_OH_mld(2), T_OH_sld(2);
    for (int i = 0; i < 2; ++i) {
        T_OH_mld[i] = (params.cfg->T_SIFS * 3 + T_BA[i] + T_DIFS[i] + 
                       params.cfg->T_RTS + params.cfg->T_CTS) / params.cfg->sigma;
        T_OH_sld[i] = (params.cfg->T_SIFS * 3 + T_BA_sld[i] + T_DIFS[i] + 
                       params.cfg->T_RTS + params.cfg->T_CTS) / params.cfg->sigma;
    }
    
    vector<double> tau_T1(2), tau_T2(2);
    tau_T1[0] = calc_tau_T(n0, L_subf_mld0, params.R[0], params.cfg->sigma, T_OH_mld[0], params.cfg->T_PH);
    tau_T1[1] = calc_tau_T(params.nmpdu_sld[0], L_subf_sld0, params.R[0], params.cfg->sigma, T_OH_sld[0], params.cfg->T_PH);
    tau_T2[0] = calc_tau_T(n1, L_subf_mld1, params.R[1], params.cfg->sigma, T_OH_mld[1], params.cfg->T_PH);
    tau_T2[1] = calc_tau_T(params.nmpdu_sld[1], L_subf_sld1, params.R[1], params.cfg->sigma, T_OH_sld[1], params.cfg->T_PH);
...
// 主计算流程 - 返回结果
Result process_BAW(int BAW, const Config& cfg, int N1, int N2, const vector<double>& nmpdu_sld, 
    ...
    // 计算开销时间
    vector<double> T_DIFS(2);
    for (int i = 0; i < 2; ++i) T_DIFS[i] = cfg.T_SIFS + cfg.aifsn * cfg.sigma;
    ...
    for (int32_t n0 = 0; n0 <= BAW; ++n0) {
        ...
        vector<double> T_BA = {calc_T_BA(n0, BAW), calc_T_BA(n1, BAW)};
        vector<double> T_BA_sld = {calc_T_BA(nmpdu_sld[0], BAW), calc_T_BA(nmpdu_sld[1], BAW)};
        
        vector<double> T_OH_mld(2), T_OH_sld(2);
        for (int i = 0; i < 2; ++i) {
            T_OH_mld[i] = (cfg.T_SIFS * 3 + T_BA[i] + T_DIFS[i] + cfg.T_RTS + cfg.T_CTS) / cfg.sigma; // 没有加T_PH
            T_OH_sld[i] = (cfg.T_SIFS * 3 + T_BA_sld[i] + T_DIFS[i] + cfg.T_RTS + cfg.T_CTS) / cfg.sigma;
        }

        vector<double> tau_T1(2), tau_T2(2);
        tau_T1[0] = calc_tau_T(n0, L_subf_mld0, R[0], cfg.sigma, T_OH_mld[0], cfg.T_PH);
        tau_T1[1] = calc_tau_T(nmpdu_sld[0], L_subf_sld0, R[0], cfg.sigma, T_OH_sld[0], cfg.T_PH);
        tau_T2[0] = calc_tau_T(n1, L_subf_mld1, R[1], cfg.sigma, T_OH_mld[1], cfg.T_PH);
        tau_T2[1] = calc_tau_T(nmpdu_sld[1], L_subf_sld1, R[1], cfg.sigma, T_OH_sld[1], cfg.T_PH);
    ...
    double T_BA0_slo = calc_T_BA(BAW, BAW);
    double T_BA1_slo = calc_T_BA(BAW, BAW);
    double T_BA_sld0_slo = calc_T_BA(nmpdu_sld[0], BAW);
    double T_BA_sld1_slo = calc_T_BA(nmpdu_sld[1], BAW);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        T_OH_mld_slo[i] = (cfg.T_SIFS * 3 + T_BA + T_DIFS[i] + cfg.T_RTS + cfg.T_CTS) / cfg.sigma;
        T_OH_sld_slo[i] = (cfg.T_SIFS * 3 + T_BA_sld + T_DIFS[i] + cfg.T_RTS + cfg.T_CTS) / cfg.sigma;
    }
    
    vector<double> tau_T1_slo(2), tau_T2_slo(2);
    tau_T1_slo[0] = calc_tau_T(BAW, L_subf_slo, R[0], cfg.sigma, T_OH_mld_slo[0], cfg.T_PH);
    tau_T1_slo[1] = calc_tau_T(nmpdu_sld[0], L_subf_sld0_slo, R[0], cfg.sigma, T_OH_sld_slo[0], cfg.T_PH);
    tau_T2_slo[0] = calc_tau_T(BAW, L_subf_slo, R[1], cfg.sigma, T_OH_mld_slo[1], cfg.T_PH);
    tau_T2_slo[1] = calc_tau_T(nmpdu_sld[1], L_subf_sld1_slo, R[1], cfg.sigma, T_OH_sld_slo[1], cfg.T_PH);
    ...
    result.mlo = (result.D_mlo > result.D_slo) && (std::abs(result.T1 - result.T2) < 5);
    result.continuous_nmpdu = {optimal_n0, optimal_n1};
    result.continuous_D_mlo = cont_D_mlo;
    result.continuous_D1 = cont_D1;
    result.continuous_D2 = cont_D2;
    result.continuous_T1 = cont_T1;
    result.continuous_T2 = cont_T2;
...
int main() {
    ...
    // double x_min = -1.0;
    // double x_max = 1.0;
    // double b = scan_cfg.R2_values[0];
    // for (int k = 0; k < N; ++k) {
    //     double x = x_min + (x_max - x_min) * k / (N - 1);
    //     double ratio = std::pow(10.0, x);   // a / b
    //     scan_cfg.R1_values.push_back(b * ratio);
    // }

    // int N = 201;
    // double x_min = -1.0;   // log10(0.1)
    // double x_max = 1.0;    // log10(10)
    // double R_sum = 2200.0;
    // scan_cfg.R1_values.clear();
    // scan_cfg.R2_values.clear();
    // for (int k = 0; k < N; ++k) {
    //     double x = x_min + (x_max - x_min) * k / (N - 1);
    //     double ratio = std::pow(10.0, x);   // R1 / R2
    //     double R1 = R_sum * ratio / (1.0 + ratio);
    //     scan_cfg.R1_values.push_back(R1);
    // }

    // for (double i = 0.01; i <= 10 + 1e-10; i += 0.01) { 
    //     scan_cfg.R1_values.push_back(scan_cfg.R2_values[0] * i); 
    // }

    // for (double i = 0; i <= scan_cfg.BAW_values[0] + 1e-9; i += 1) {
    //     scan_cfg.nmpdu_sld0_values.push_back(i);
    // }
        // 创建总CSV文件
    string summary_filename = "changer1r2-inter.csv";
    ...
    summary_csv << "R1,R2,N1,N2,nmpdu_sld0,nmpdu_sld1,BAW,"
                   ...
                   "Continuous_nmpdu_0,Continuous_nmpdu_1,"
                   "Max_nmpdu_mld_0,Max_nmpdu_mld_1,"
                   "D1,D2,D,T1,T2,"
                   "Continuous_D1,Continuous_D2,Continuous_D,Continuous_T1,Continuous_T2,"
                   "p,alpha1,alpha2,lambda_out1,lambda_out2,D_slo,lambda_out_slo,mlo\n";
    ...
    for (double R1 : scan_cfg.R1_values) {
        for (double R2 : scan_cfg.R2_values) {
            for (double nmpdu_sld0 : scan_cfg.nmpdu_sld0_values) {
                for (double nmpdu_sld1 : scan_cfg.nmpdu_sld1_values) {
                    for (int N1 : scan_cfg.N1_values) {
                        for (int N2 : scan_cfg.N2_values) {
                            // double R2 = R_sum - R1;
                            // int N2 = 10 - N1;
                            // double nmpdu_sld1 = scan_cfg.BAW_values[0] - nmpdu_sld0;
                            vector<double> tau_F(2);
                            vector<int32_t> nmpdu_mld_max(2);
                            vector<double> R = {R1, R2};
                            for (int i = 0; i < 2; ++i) {
                                tau_F[i] = (cfg.T_RTS + cfg.T_SIFS + cfg.aifsn * cfg.sigma) / cfg.sigma;
                                // 计算nmpdu_mld_max
                                int32_t max_nmpdu = 1;
                                int32_t left = 1, right = 10000;
                                while (left <= right) {
                                    int32_t mid = (left + right) / 2;
                                    double payloadDuration = calc_payload_duration(mid, cfg.L_subf, R[i]);
                            ...
                            auto p1 = solve_p_simplified(nmpdu_sld0 ? N1 : 0.0, cfg.K, cfg.K, cfg.W[0], cfg.W[1]);
                            auto p2 = solve_p_simplified(nmpdu_sld1 ? N2 : 0.0, cfg.K, cfg.K, cfg.W[0], cfg.W[1]);
                            ...
                            for (int BAW : scan_cfg.BAW_values) {
                                ...
                                try {
                                    vector<double> nmpdu_sld = {nmpdu_sld0, nmpdu_sld1};
                                    Result result = process_BAW(BAW, cfg, nmpdu_sld0 ? N1 : 0, nmpdu_sld1 ? N2 : 0, nmpdu_sld, p1, p2, tau_F, R);
                                    result.nmpdu_mld_max = nmpdu_mld_max;

                                    // 写入总CSV
                                    summary_csv << result.R[0] << "," << result.R[1] << ","
                                            << N1 << "," << N2 << ","  << result.nmpdu_sld[0] << "," << result.nmpdu_sld[1] << ","
                                            << result.BAW << "," << result.best_nmpdu[0] << "," << result.best_nmpdu[1] << ","
                                            << result.continuous_nmpdu[0] << "," << result.continuous_nmpdu[1] << ","
                                            << result.nmpdu_mld_max[0] << "," << result.nmpdu_mld_max[1] << ","
                                            << result.D1 << "," << result.D2 << "," << result.D_mlo << ","
                                            << result.T1 << "," << result.T2 << ","
                                            << result.continuous_D1 << "," << result.continuous_D2 << "," << result.continuous_D_mlo << ","
                                            << result.continuous_T1 << "," << result.continuous_T2 << ","
                                            << result.p << ","<< result.alpha1 << "," << result.alpha2 << ","
                                            << result.lambda1 << "," << result.lambda2 << ","
                                            << result.D_slo << "," << result.lambda_slo << "," << result.mlo << "\n";
                                } catch (const exception& e) {
